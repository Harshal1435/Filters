...
if (click!=true) {
    ...
    fil.style.marginLeft="0%"
    click=true;
    fil.style.boxShadow="rgba(0,0,0,0.35) 5px 10px 15px"
}
else {
      var fil=document.getElementById("filters");
    fil.style.marginLeft="-45%"
    click=false;
    fil.style.boxShadow="none"
}
}
...
items.classList.add("product_items")

let containerItems=document.createElement("div")
containerItems.classList.add('card')
items.appendChild(containerItems)
let propic=document.createElement("div")
propic.classList.add("pro_pic")
containerItems.appendChild(propic)
let proimg=document.createElement("img")
proimg.setAttribute("src", product.pic)
propic.appendChild(proimg)

let proinfo=document.createElement("div")
proinfo.classList.add("pro_info")
...
let filters ={
    ...
    function filterData() {
      ...
      if (filters.product_name !== null) {
        filterArr = filterArr.filter((product,value) => {
